...
if __name__ == "__main__":
    ...
    if len(model_holder.model_names) == 0:
        ...
        logger.error(f"Models not found in {model_holder}.")
        sys.exit(1)
    ...
    logger.info(f"server listen: http://0.0.0.1:{config.server_config.port}")
    logger.info(f"API docs: http://0.0.0.1:{config.server_config.port}/docs")
    uvicorn.run(
